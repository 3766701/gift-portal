...
const queryActivationCode=async code=>{if(!code){showResultModal('请输入激活码。');return}try{const r=await fetch(api('/api/orders/'+encodeURIComponent(code))),d=await r.json();if(r.status===404){showResultModal('对不起,查询不到该激活码/订单号!');return}if(d.status==='未领取'){showResultModal('该激活码还未提货，请先提交后，再查询结果哦!');return}if(d.status==='已领取'){showCompletedQueryResult(d.code||code,d.reward);return}showResultModal(d.message||'查询失败，请稍后重试。')}catch{showResultModal('查询失败，请确认后端已启动。')}};
...
$('#query-order').addEventListener('click',()=>{queryActivationCode($('#order-id').value.trim())});
