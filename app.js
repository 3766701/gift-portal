...
$('#check-order-short').addEventListener('click',()=>{$('#order-id').value=$('#code').value.trim();activateTab('orders')});
$('#query-order').addEventListener('click',async()=>{const code=$('#order-id').value.trim(),out=$('#order-result');if(!code){showResultModal('请输入激活码。');return}out.textContent='';try{const r=await fetch(api('/api/orders/'+encodeURIComponent(code))),d=await r.json();showResultModal(d.status==='未领取'?'该激活码还未提货，请先提交后，再查询结果哦!':d.message||`${d.status} · ${d.reward}`)}catch{showResultModal('查询失败，请确认后端已启动。')}});
