...
const saveRuntimeSettings=async(refresh_nodes=false,action='')=>{setMessage('runtime-message');try{await request('/api/admin/runtime-settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({seed_proxies:[el('seed-proxy-1').value.trim(),el('seed-proxy-2').value.trim()].filter(Boolean),seed_capacity:el('seed-capacity').value,proxy_group:el('clash-proxy-group').value.trim(),test_url:el('clash-test-url').value.trim(),refresh_nodes,switch_node:action==='switch',best_node:action==='best'})});await loadRuntimeStatus();setMessage('runtime-message',refresh_nodes?'节点已重新测速。':action?'线路已切换。':'运行配置已保存。');}catch(error){if(error.status===401){showLogin();return;}setMessage('runtime-message',error.message);}};
